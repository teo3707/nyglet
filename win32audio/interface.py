...
import ctypes
import weakref
from win32audio import lib_dsound as lib
...
def _check(hresult):
    if hresult != lib.DS_OK:
        raise Exception('DirectSoundNativeError')
...
class DirectSoundDriver(object):
    def __init__(self):
        ...
        # A trick used by mplayer.
        # use desktop as window handle since it would be complex to use
        # pyglet window handles (and what to do when application is audio only?).
...
class DirectSoundBufferFactory(object):
    ...
    def create_buffer(self, audio_format):
        ...
        return DirectSoundBuffer(self._create_buffer(buffer_desc),
                                 audio_format,
                                 buffer_size)

    def create_primary_buffer(self):
        return DirectSoundBuffer(self._create_buffer(self._create_primary_buffer_desc()),
                                 None,
                                 0)
    ...
    def _create_wave_format(audio_format):
        ...
        wfx.nSamplePerSec = audio_format.sample_rate
        wfx.wBitsPerSample = audio_format.sample_size
        wfx.nBlockAlign = wfx.wBitsPerSample * wfx.nChannels // 8
        wfx.nAvgBytesPerSec = wfx.nSamplePerSec * wfx.nBlockAlign
        return wfx
...
class DirectSoundBuffer(object):
    ...
    def current_position(self):
        ...
        Only play position can be modified, so setter only accepts a sing value."""
    ...
    def frequency(self):
        value = lib.DWORD()
        _check(self._native_buffer.GetFrenquency(value))
        return value.value

    @frequency.setter
    def frequency(self, value):
        """The frequency, in samples per seconds, at which the buffer is playing."""
    ...
    def cone_outside_volume(self):
        """The volume on the sound outside angle of the sound projection cone."""
        if self.is3d:
            volume = lib.LONG()
    ...
    def lock(self, write_cursor, write_size):
        ...
        _check(self._native_buffer.Lock(write_cursor,
                                        write_size,
    ...
    def unlock(self, pointer):
        _check(self._native_buffer.Unlock(pointer.audio_ptr_1,
                                          pointer.audio_length_1,
                                          pointer.audio_ptr_2,
                                          pointer.audio_length_2))
...
class DirectSoundListener(object):
    def __init__(self, ds_buffer: DirectSoundBuffer, native_listener):
